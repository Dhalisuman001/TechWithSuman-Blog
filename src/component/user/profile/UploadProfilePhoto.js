...
import { uploadPhotoAction } from "../../../redux/slices/users/userSlices";
...
export default function UploadProfilePhoto() {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      dispatch(uploadPhotoAction(values));
    },
    validationSchema: formSchema,
  });
  //store data
  const users = useSelector(state => state?.users);
  const { uploadedPhoto, loading, appErr, serverErr, userAuth } = users;
  //redirect
  if (uploadedPhoto) return <Navigate to={`/profile/${userAuth?._id}`} />;
  return (
