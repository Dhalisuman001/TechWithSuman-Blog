...
import { emailSentAction } from "../../../redux/slices/email/EmailSlices";
...
const SendEmail = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
      //dispath the action
       dispatch(emailSentAction(values));
    },
    validationSchema: formSchema,
  });
  const mail = useSelector(state=>state.email);
  const {isSend,loading} = mail;
  // console.log(isSend);
  if(isSend) return <Navigate to={`/profile/${id}`}/> ;
  return (
